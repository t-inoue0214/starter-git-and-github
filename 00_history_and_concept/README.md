...
- **GitHubはGitがないと使えません。** オンラインサーバーだけあっても、ゲーム機（Git）がないと遊べないのと同じです。
...
**重要:** どのサービスを使っていても、 **使う「Gitコマンド」としては同じ** です。

「プッシュ先（URL）」が変わるだけで、Git操作としては、やることは変わりません。GitHubで基本を覚えれば、どこに行っても通用します。

違いがあるとすれば、[# 【資料】ローカル環境での認証設定](../99_reference/authentication.md) に記載しているような認証の仕方がそれぞれ異なりますので、環境ごとに調べるようにしてください。
